              ...
              ElevatedButton(
                onPressed: () async {
                  if (_formKey.currentState?.validate() ?? false) {
                    // Convert course capacity to integer
                    final int? capacity =
                        int.tryParse(coursecapacity.text.trim());

                    if (capacity == null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Invalid course capacity')),
                      );
                      return;
                    }

                    final courseType =
                        _character == CourseType.offline ? 'Offline' : 'Online';
                    ...
                    int result = await _databaseHelper.registerCourse(course);
                    if (result > 0) {
